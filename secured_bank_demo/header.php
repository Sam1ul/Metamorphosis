...
// Load background only if user exists
if ($u) {
    $db = db();
    ...
    if ($bg_db) {
        ...
        // Allow only HTTPS URLs (no javascript:, data:, etc.)
        if (filter_var($bg_db, FILTER_VALIDATE_URL)) {

            $parsed = parse_url($bg_db);
        ...
        //     $safePath = realpath(__DIR__ . '/uploads/' . basename($bg_db));

        //     if ($safePath && strpos($safePath, realpath(__DIR__ . '/uploads')) === 0) {
        //         $bg = 'uploads/' . basename($bg_db);
        //     }
        // }
            } else {
                  // Allow files inside uploads directory (including subfolders)
                  $safePath = realpath(__DIR__ . '/' . $bg_db);

                  if ($safePath && strpos($safePath, realpath(__DIR__ . '/uploads')) === 0) {
                      $bg = $bg_db; // keep full relative path
                  }
              }        
    }
}
...
<body class="relative min-h-screen flex flex-col bg-cover bg-center bg-no-repeat"
      style="background-image:url('<?php echo htmlspecialchars($bg, ENT_QUOTES, 'UTF-8'); ?>');">
      ...
      <div class="flex items-center justify-between py-3">
        ...
        <div class="flex items-center">
          ...
          <div class="text-blue-900 mr-3 text-sm">
            Signed in as <strong><?php echo htmlspecialchars($u['username'], ENT_QUOTES, 'UTF-8'); ?></strong>
          </div>
  ...
  <div class="bg-white rounded p-6 max-w-md w-full">
    ...
    <form method="POST" action="preview_background.php" class="flex flex-col gap-3">
      ...
      <div class="flex gap-2 justify-end">
        ...
        <button type="button"
          onclick="document.getElementById('bgModal').classList.add('hidden')"
